...
interface ProgressBarWithShipProps {
  pct: number;
  color: string;
}

function TinyPirateShip({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 60 40" fill="none" xmlns="http://www.w3.org/2000/svg" width="28" height="19">
      {/* Hull */}
      ...
      {/* Cannon ports */}
      <circle cx="18" cy="28" r="1.2" style={{ fill: color }} opacity={0.6} />
      <circle cx="30" cy="28.5" r="1.2" style={{ fill: color }} opacity={0.6} />
      <circle cx="42" cy="28" r="1.2" style={{ fill: color }} opacity={0.6} />
    </svg>
  );
}

export default function ProgressBarWithShip({ pct, color }: ProgressBarWithShipProps) {
  return (
    <div className="relative" style={{ paddingTop: "22px" }}>
      {/* Ship */}
      <motion.div
        className="absolute"
        style={{ bottom: "-3px", zIndex: 10 }}
        initial={{ left: "0%" }}
        animate={{ left: `${pct}%` }}
